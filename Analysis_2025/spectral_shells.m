function [Eshell, Emod] = spectral_shells(kx, ky, kz, E, N, kmax, space_type)
    ...
    for i = 1:length(kmod_1D)
        ...
        Eshell{c}.spec = E(mask);

        % Compute the mean spectral energy in the shell for Emod
        Emod.kmod(c) = kmod_1D(i);
        Emod.spec(c) = 4*pi*(kmod_1D(i)^2)*abs(mean(Eshell{c}.spec));
